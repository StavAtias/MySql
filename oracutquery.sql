...
from (select count(*) 'painting_times'
   ...
   -- most backup --                 
  select a.id, s.first_name ,s.last_name , m.first_name 'manager' , b.id 'barbershop' , a.times                  
  from (select s.id 'id' , count(*) 'times' , s.barber_shop_id 'barber_shop'
		from staff s  
		group by s.backup_id
        order by count(*) desc
        limit 1) a, staff s , manager m , barber_shop b 
  where m.id = b.manager_id
  and b.id = a.barber_shop
  and s.id = a.id;
